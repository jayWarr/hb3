...
#################################################################
# IMPORTANT                                                     #
# This utility makes use of 3 files in a sub-directory named    #
# hummingbird located in the Downloads directory                #
#################################################################
...
# DISPLAY_FILENAME =  r'/Users/jr/adv/hb3/hummingbird3/_utilities_/getDocuments_2.png'  # if you want to use a file instead of data, then use this in Image Element
DISPLAY_TIME_MILLISECONDS = 4000
VERSION =           'version: v0.6.1 14-Sep-21'
...
SETTINGS_FILE       =     path.join(DOWNLOADS_PATH, '__hb_settings.cfg')
...
PM=                            {('error',   'background'):  'red',
                                ...
                                ('info',    'text'):        'black',
                                ('help',    'background'):  'green',
                                ('help',    'text'):        'white',
                                ('special', 'background'):  'purple',
                                ('special', 'text'):        'white',
                                }
CATEGORIES_FILE_CP_YMDHMS =     path.join(DOWNLOADS_PATH, r'__hb_categoriesCP_YMDHMS.cfg')
CATEGORIES_FILE_PC_YMDHMS =     path.join(DOWNLOADS_PATH, r'__hb_categoriesPC_YMDHMS.cfg')
DEFAULT_CATEGORIES= {'*':                ['Life','Humanities','Natural World','Sciences','The Inexplicable'],
                     ...
                     'Love':             ['Partner','Other'],
                     'Humanities':       ['Art','Performing arts','Literature','Philosophy','Poetry','Religion','Classics'],
...
def getLatestCatFile(cp):
    # get the latest cat files
    fileList=fnmatch.filter(os.listdir(DOWNLOADS_PATH),u'__hb_categories%s_*.cfg' %  cp)
    if len(fileList)==0:
        # none found
        return '',False
    else:
...
def load_categories():
    ...
    CATEGORIES_FILE_CP, foundCP = getLatestCatFile('CP')
    CATEGORIES_FILE_PC, foundPC = getLatestCatFile('PC')
    if foundCP and foundPC:
        with open(CATEGORIES_FILE_CP, 'r') as f:
            categoriesCP=jsonload(f)
        with open(CATEGORIES_FILE_PC, 'r') as f:
            categoriesPC=jsonload(f)
        pathH,pathT= os.path.split(CATEGORIES_FILE_PC)
        filename, extension =os.path.splitext(pathT)
        bits= filename.split("_")
        dt=bits[-1]
        dtExpanded=u'%s-%s-%s @ %s:%s:%s' % ( dt[0:2], dt[2:4], dt[4:6], dt[6:8], dt[8:10], dt[10:12] )
    else:
        popupMessage('info', u'No category files found...\n'
                             u'they will be created in your Downloads sub-dir')
        t_categories={}
...
def save_categories(categoriesCP,categoriesPC,newCategories):
    ...
    CATEGORIES_FILE_CP  = CATEGORIES_FILE_CP_YMDHMS.replace('YMDHMS',dtSuffix)
    CATEGORIES_FILE_PC  = CATEGORIES_FILE_PC_YMDHMS.replace('YMDHMS',dtSuffix)
    with open(CATEGORIES_FILE_CP, 'w') as f:
        jsondump(categoriesCP, f)
    with open(CATEGORIES_FILE_PC, 'w') as f:
        jsondump(categoriesPC, f)
    popupMessage('info',u"The categories have been saved in: \n%s\nin files with the suffix '%s'" % (DOWNLOADS_PATH,  dtSuffix))
    return dtExpanded
...
def showCatTree(catPC,catCP, subCats, catUpdates, catLastUpdated):
    ...
    treeLayout = [  [TextLabel('The categories tree of relationships & assignment counts', FONT_REGULAR_SIZE)],
                    ...
                    [ Btn('Remove','-REMOVE-') ],
                    [ TextLabel('and to:',FONT_REGULAR_SIZE), Btn('Rename','-RENAME-'),TextLabel(' or ',FONT_REGULAR_SIZE), Btn('Add','-ADD-')],
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, '-CLOSE-'):
            ...
            popupMessage('help','Catgegories will be initialised automatically\n'
                                'with files downloaded from the application.\n\n'
                                'In the absence of files a default tree will\n'
                    ...
                    for subCat in subCats:
                        if theNewSubCat in subCat:
                            popupMessage('info', u"The category '%s'\n you want to %s already exists!\n"                           
                                                 u"Suggest you add a suffix like _Number and resubmit" % (theNewSubCat,rca))
                            nbg=True
                            break
                        ...
                        if theHC in catPC[majorCat]:
                            ...
                            if count>0 :
                                popupMessage('warning', 'Uanble to %s the category\n\n'
                                                        "'%s'\n\n"
                                                        "because it has %s title%s assigned ?!" % (rca,theHC,count,pluralize(count)))
                                nbg=True
                            else:
    ...
    if values:      # if there are stuff specified by another window, fill in those values
        for key in SETTINGS_KEYS_TO_ELEMENT_KEYS:  # update window with the values read from settings file
            try:
                ...
                elif theValue=='-STARTING_DAY-' or theValue=='-ENDING._DAY-' or theValue=='-STARTING_YEAR-' or theValue=='-ENDING._YEAR-':
...
def popupMessage(type,msg):
    sg.popup( msg, title=u'%s - %s' % (HB_UTILITY_TITLE, type) ,keep_on_top=True, line_width=80,
              background_color=PM[(type,'background')],text_color=PM[(type,'text')],font=(FONT_REGULAR, FONT_REGULAR_SIZE))
...
def Rb(text,groupKey,number, theDefault): return sg.Radio(text,groupKey, default=theDefault,
                                font=(FONT_REGULAR, FONT_REGULAR_SIZE), key=(u'-%s%s-' % (groupKey.upper(),number)) )
...
def Btn(text,theKey=None, isVisible=True, highlight=False):
    if theKey==None:
        ...
        k=u'%s' % theKey.upper()
    if highlight:
        return sg.Button(text, key=k, visible=isVisible, font=(FONT_REGULAR, FONT_REGULAR_SIZE), button_color=(boldColours()))
    else:
...
def testForNoneFound(type, NoOf):
    if NoOf == 0:
        popupMessage('warning','Terminating the utility !\n\nNo %s have been found ?!' % type)
        sys.exit()
    return
...
#-----------------------------------------------------------------------------------------------------------------------
    if verboseComments:
        ...
        layout = [
                ...
                [TextLabel('5. Categorize titles')],
                [TextLabel('6. Create the upload file')],
                [bL()],
                [TextLabel('The NEXT button will always take you to the next step.')],
                [bL()],
                [Btn("Next",'-NEXT-', True, True), Btn("About", '-ABOUT-') ,Btn("Quit",'-QUITTING-')]
            ]
        window=sg.Window(HB_UTILITY_TITLE, layout, size=(450,520), keep_on_top=True)
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    updateButton=Btn("Update",'-UPDATE-',True,True)
    now = datetime.now()
    ...
    layout = [[theTextTitle('1. Review, revise and update the default settings')],
              [sg.Frame(layout=[
                  ...
                  [sg.Input(sDIR,key="-FOLDER-", size=(60, 1), font=(FONT_REGULAR, FONT_REGULAR_SIZE)), sg.FolderBrowse()],
                  [bL()],
                  [TextColourLabel('Have the files selected by:'), Rb('automatically retrieving↩︎︎︎', 'select', 1, selectFilesAuto),
                   Rb('manually browsing', 'select', 2, selectFilesMan)],
                  [bL()],
                  [TextColourLabel('Limit the number of files to a maximum of: '),
                   sg.InputCombo(values=NUMBERS1, font=(FONT_REGULAR, FONT_REGULAR_SIZE), size=(5, 10), key=("-NUMBER1-"),
                                 default_value= limitFilesRetrived ),
                   TextColourLabel('ignoring the first: '),
                   sg.InputCombo(values=NUMBERS1, font=(FONT_REGULAR, FONT_REGULAR_SIZE), size=(5, 10), key=("-NUMBER3-"),
                                 default_value=ignoreTheFirst)
                   ],
                  [bL()],
                  [TextColourLabel('within the period: ')], datePicker('starting',startingDay, startingMonth, startingYear),
                  ...
                  [Cb('*.txt',      "EXTN", 0, settings['extn0']),
                   ...
                   Cb('*.pages✗︎',   "EXTN", 7, settings['extn7'])],
                  [bL()],
                  ...
                  [Cb('with verbose information.', "VERBOSE", 0, verboseComments)],
                  [bL()],
                  [sg.Text('↩︎︎ will include the files in sub-dirs recursively',
                           font=(FONT_SMALL, FONT_SMALL_SIZE))],
                  [sg.Text('✗︎︎ these document types are not readable - titles default to filenames',
                           font=(FONT_SMALL, FONT_SMALL_SIZE))],
                  [bL()],
              ],
              ...
              [bL()],
              [TextLabel('Theme'),sg.InputCombo(values=sg.theme_list(), size=(20, 20), key='-THEME-', default_value=theTheme  ,font=(FONT_REGULAR, FONT_REGULAR_SIZE))],
              [bL()],
              [bL()],
              [updateButton, nextButton, Btn('Help', '-HELP_REVISE_SETTINGS-'), Btn("Quit", '-QUITTING-')]
            ]

    window = sg.Window(HB_UTILITY_TITLE, layout, size=(670, 840),  finalize=True)
    ...
    while True:
        ...
        if Quitting(event,"If not UPDATED you will loose any changed settings." ):
            ...
            if new_sDIR == None:
                ...
                popupMessage('warning', u'[ %s ] (sub)directory does not exist ?! %s' % (new_sDIR, SETTINGS_NOT_UPDATED))
            ...
            if ignoreTheFirst>limitFilesRetrived:
                popupMessage('warning',"Can't ignore more (%s) files than the limit (%s)" % (ignoreTheFirst, limitFilesRetrived ))
                canUpdateSettings = False
            ...
            if listExtns=='':
                canUpdateSettings=False
                popupMessage('warning',u'Poem files MUST be filtered by at least one extension type. %s' % SETTINGS_NOT_UPDATED)
            ...
            if preference>=2:
                ...
                for nvE in nvEXTNS:
                    if nvE in extnsList:
                        nvEs.append(nvE)
                if len(nvEs)!=0:
                    popupMessage('warning',u'%s for files with extensions: %s.\n\nFilenames will be used for titles.' % (vt, nvEs ))
            ...
            updateButton(button_color=( reverseColour(sg.theme_background_color()), reverseColour(sg.theme_text_color())))
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    nextButton=Btn('Next', '-NEXT-',False)
    numberofTitles=0
    layout0 = [[theTextTitle('3. Filter the %s file%s and extract their title%s' % (nf, pluralize(nf),pluralize(nf)))],
              [bL()],
              [bL()],
              [bL()],
              [bL()],
    ...
    while True:
        ...
        if Quitting(event, "You will loose any files retrieved."):
            ...
            popupMessage('help','\n\nFltering & title extractio nmust be performed before the next step.\n\n'
            ...
            if verboseComments:
                popupMessage('info', u'From the filtered files:\n\n'
                                     ...
                                     u'%s\n' % (numberofTitles, pluralize(numberofTitles),
                                              numberofdefault2Filename, was_were(numberofdefault2Filename), its_their(numberofdefault2Filename),pluralize(numberofdefault2Filename),
                                              default2Filename))
            ...
            window0['-TITLE-'].update(u'3. Filtered the files and extracted their %s title%s' %  (numberofTitles, pluralize(numberofTitles)))
...
#-----------------------------------------------------------------------------------------------------------------------
            ...
            if checkUploadTitlesCount!=0 and verboseComments:
                popupMessage('special', u'Have removed %s title%s from current list\n'
                                         '%s already in the waiting upload files' % (checkUploadTitlesCount,
                                                                                  pluralize(checkUploadTitlesCount),
                                                                                  itIs_theyAre(checkUploadTitlesCount))
    ...
    layout = [[theTextTitle('4. Review and removed unwanted titles')],
              [bL()],
              [sg.Listbox(values=list(td.keys()), key='-SELECTED-', size=(50, 20), font=(FONT_REGULAR, FONT_REGULAR_SIZE),
              ...
              [sg.Input(key='_HIDDEN_', visible=False, enable_events=True, font=(FONT_REGULAR, FONT_REGULAR_SIZE)),
                        Btn('Remove')],
              [bL()],
              [TextLabel(u'%s titles have been removed.' % noOfTitlesRemoved, FONT_REGULAR_SIZE,'-NTITLESREMOVED-')],
              [bL()],
              [bL()],
              [sg.pin(finishReviewButton), sg.pin(categorizeButton), sg.pin(nextButton), Btn('Help', '-HELP_REVIEW-'), Btn("Quit", '-QUITTING-')]
              ]
        ...
        if Quitting(event, "You will loose any titles reviewed"):
            ...
            if noOfTitles2BRemoved == 0:
                ...
                window['-NTITLESREMOVED-'].update(u'%s title%s %s been removed.' % (noOfTitlesRemoved,
                                                                                      pluralize(noOfTitlesRemoved), has_have(noOfTitlesRemoved)))
            ...
            if verboseComments:
                popupMessage('info',u'There %s %s title%s after the review' % (is_are(theFinalNoOfTitles),
                                                                    theFinalNoOfTitles, pluralize(theFinalNoOfTitles)))
            titlesReviewed  = True
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    if lastEvent=='-CATEGORIZE-':
        ...
        layout = [[theTextTitle('5. Categorize titles')],
                  [bL()],
                  [bL()],
                  [TextLabel(u'There %s %s title%s to be categorized' % (is_are(theFinalNoOfTitles),theFinalNoOfTitles,
                                pluralize(theFinalNoOfTitles)),FONT_REGULAR_SIZE,'-NTITLES2BCATEGORIZED-')],
                  [bL()],
                  ...
                  [bL()],
                   ...
                   TextLabel('with'),
                   sg.InputCombo(values=subCats, font=(FONT_REGULAR, FONT_REGULAR_SIZE), size=(35, 10), key=("-SUBandMAJORCATS-"))
                   ],
                  [bL()],
                  ...
                  [bL()],
                  [Btn('Tree','-TREE-'), Btn('Finish', '-FINISH CATEGORIZING-', True, True), Btn('Help', '-HELP_CATEGORIZE-'),
                   Btn("Quit", '-QUITTING-')]
                  ]
        ...
        while True:
            ...
            if Quitting(event, "You will loose the titles"):
                ...
                popupMessage('help', 'Categorization is optional:\n'
                                     ...
                                     'The titles will be prefixed with ▶︎ , suffixed with the\n'
                ...
                categoriesPC,categoriesCP, subCats, catUpdates, catLastUpdated  =showCatTree (categoriesPC,categoriesCP, subCats, catUpdates, catLastUpdated)
                ...
                if noOfTitles2BCat == 0:
                    ...
                    window['-NTITLES2BCATEGORIZED-'].update(u'There %s %s title%s to be categorized' % (is_are(n2BC),
                                                              n2BC, pluralize(n2BC)))
                    window['-NTITLESCATEGORIZED-'].update(u'There %s %s title%s categorized' % (is_are(nCategorized),
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    testForNoneFound('titles', theFinalNoOfTitles)
    # generate upload filename with yearMonthDayHourMinSec suffix to ensure uniqueness
    nowDT = datetime.now()
    ...
    layout = [[theTextTitle('5. Create the upload file and finish')],
                [bL()],
                [bL()],
                [TextLabel(u'There %s %s title%s and %s category update%s for the upload file:' % (is_are(theFinalNoOfTitles),theFinalNoOfTitles
                            ,pluralize(theFinalNoOfTitles), noOfCategoryUpdates, pluralize(noOfCategoryUpdates)))],
                [bL()],
                [TextColourLabel(hbUploadFilename)],
                [bL()],
                [bL()],
                [Btn('Create upload','-CREATE-', True, True), Btn('Help', '-HELP_CREATE-'),Btn("Quit", '-QUITTING-')]
              ]
    ...
    while True:
        ...
        if Quitting(event, "You will loose the upload file about to be created."):
            ...
            window.close()
            if verboseComments:  popupMessage('info', filterMsg)
            lines = HB_TITLE
            if noOfCategoryUpdates>0:
                lines+= AND_CATEGORY_UPDATES
            lines +=['...',
                            ...
                            u'... %s' % sDIR,
                            u'... This file includes %s category update%s.' % (noOfCategoryUpdates, pluralize(noOfCategoryUpdates)),
                            '...',
